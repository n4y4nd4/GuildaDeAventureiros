...
@Table(schema = "audit", name = "api_keys",
       uniqueConstraints = @UniqueConstraint(name = "uq_api_keys_nome_por_org",
           columnNames = {"organizacao_id", "nome"}))
public class ApiKey {

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "api_keys_seq")
    @SequenceGenerator(name = "api_keys_seq", sequenceName = "audit.api_keys_id_seq", allocationSize = 1)
    private Long id;
